...
public class DateTest : MonoBehaviour
{
    private void Start()
    {
        Dictionary<string, string[]> EndingCreditsList = CSVWidthCutting(CSVLoader(Define.CSV.EndingCredits));
        Dictionary<string, string[]> LanguagePackList = CSVLengthCutting(CSVLoader(Define.CSV.LanguagePack));
        // 딕셔너리의 모든 항목을 출력합니다.


        //foreach (KeyValuePair<string, string[]> item in EndingCreditsList)
        //{
        //    string t = "";
        ...
        //        t += value.ToString() + "  ";
        //    }
        //    Debug.Log("Key: " + item.Key + "Value: " + t);
        //}

        foreach (KeyValuePair<string, string[]> item in LanguagePackList)
        {
            string t = "";
            foreach (string value in item.Value)
            {
        ...
        Dictionary<string, string[]> list = CSVSortationCutting(FindValuesByKey(LanguagePackList, Define.Language.Korean), typeof( Define.LanguageDialogue));
    ...
    //void Start()
    //{
    //    Debug.Log( $"{Lord().stageClear}  {Lord().playStage}  {Lord().LastCheckPointToVector3()}  {Lord().NowPlayerPointToVector3()}  {Lord().isPlaying}");
